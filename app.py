...
# ----------------------------------------------------------------------
# Installed app & game scanning
#
# CleanSlate looks for installed apps the same way Windows' own
# "Apps & Features" panel does - by reading the Uninstall registry keys -
# rather than crawling every file on disk, which would be slow and would
# also pick up a lot of junk that isn't really a standalone "app".
# ----------------------------------------------------------------------
def _scan_windows_registry_apps():
    apps = []
    if os.name != "nt":
        return apps
    try:
    ...
    skip_release_types = {"Update", "Hotfix", "Security Update", "Update Rollup"}
    seen = set()
                ...
                try:
                    with winreg.OpenKey(root_key, sub_name) as sub_key:
                        ...
                        if _get("SystemComponent") == 1:
                            continue
                        if _get("ReleaseType") in skip_release_types:
                            continue
                        if _get("ParentKeyName"):
                            continue  # this is a sub-component of another product
                        ...
                        apps.append({
                            "name": name,
                            "version": str(_get("DisplayVersion") or "").strip(),
                            "publisher": str(_get("Publisher") or "").strip(),
                            "install_location": str(_get("InstallLocation") or "").strip(),
                            "category": "app",
                        })
                except OSError:
                    continue
...
def _find_steam_path():
    ...
    try:
        ...
        pass
    default = os.path.join(os.environ.get("ProgramFiles(x86)", r"C:\Program Files (x86)"), "Steam")
    return default if os.path.isdir(default) else None


def _scan_steam_games():
    """Steam remembers your library once you sign back in, so this is
    mostly a 'here's what you had' list rather than something that needs
    copying - Steam re-downloads the actual game files itself."""
    games = []
    ...
    for lib in library_folders:
        ...
        for fn in entries:
            if not (fn.startswith("appmanifest_") and fn.endswith(".acf")):
                continue
            try:
                with open(os.path.join(steamapps_dir, fn), "r", encoding="utf-8", errors="ignore") as f:
                    content = f.read()
                ...
                seen.add(key)
                games.append({
                    "name": name, "version": "", "publisher": "Steam",
                    "install_location": steamapps_dir, "category": "game",
                })
            except Exception:
                continue
    return games
    ...
    for base in ("/Applications", os.path.expanduser("~/Applications")):
        ...
        for name in entries:
            if not name.endswith(".app"):
                continue
            app_name = name[:-4].strip()
            key = app_name.lower()
            if key in seen or not app_name:
                continue
            seen.add(key)
            apps.append({
                "name": app_name, "version": "", "publisher": "",
                "install_location": os.path.join(base, name), "category": "app",
            })
    return apps


def _scan_linux_apps():
    """Works for both plain Linux and ChromeOS's Linux (Crostini)
    container - though on ChromeOS this only sees Linux apps installed
    inside that container, not ChromeOS/Android/PWA apps, which aren't
    exposed to it."""
    apps = []
    seen = set()
    bases = (
    ...
    for base in bases:
        ...
        for fn in entries:
            if not fn.endswith(".desktop"):
                continue
            try:
                with open(os.path.join(base, fn), "r", encoding="utf-8", errors="ignore") as f:
                    content = f.read()
            except Exception:
                continue
            if re.search(r'^NoDisplay=true', content, re.MULTILINE | re.IGNORECASE):
                continue
            ...
            seen.add(key)
            apps.append({
                "name": name, "version": "", "publisher": "",
                "install_location": "", "category": "app",
            })
    return apps


def get_installed_apps():
    """Scan the computer for installed apps (and Steam games, on Windows).
    Runs on a background thread from the UI side since this can take a
    moment on machines with a lot of software installed."""
    if PLATFORM == "windows":
        return _scan_windows_registry_apps() + _scan_steam_games()
    if PLATFORM == "mac":
        return _scan_mac_apps()
...
class StepDots(tk.Frame):
    """A minimal dot progress strip, no clutter, no labels."""

    def __init__(self, parent, current_index, total=5, bg=COLOR_BG):
...
class UsbDriveIcon(tk.Canvas):
    """A simple hand-drawn USB drive icon, since color emoji glyphs (like
    the floppy disk emoji) come with their own baked-in colors that we
    can't control - on most systems that renders mostly black/dark grey,
    which disappears against a dark background. This is drawn entirely in
    our own theme colors so it's always visible."""

    def __init__(self, parent, size=96, bg=COLOR_BG):
        super().__init__(parent, width=size, height=size, bg=bg,
                          highlightthickness=0, bd=0)
        s = size
        # Drive body
        body_pts = _round_rect_points(s * 0.22, s * 0.40, s * 0.78, s * 0.90, s * 0.08)
        self.create_polygon(body_pts, smooth=True, fill=COLOR_TEXT, outline="")
        # USB connector (the metal tip)
        connector_pts = _round_rect_points(s * 0.40, s * 0.14, s * 0.60, s * 0.44, s * 0.03)
        self.create_polygon(connector_pts, smooth=True, fill=COLOR_BLUE_HINT, outline="")
        # Little indicator light on the body
        self.create_oval(s * 0.56, s * 0.66, s * 0.68, s * 0.78, fill=COLOR_PRIMARY, outline="")
        # A small sparkle accent, top-right
        cx, cy, r = s * 0.86, s * 0.20, s * 0.09
        star_pts = [
        ...
        self.create_polygon(star_pts, smooth=True, fill=COLOR_BLUE_HINT, outline="")
...
class CleanSlateApp(tk.Tk):
    def __init__(self):
        ...
        self.available_apps = []      # apps/games found by the last scan
        self.selected_app_names = set()
    ...
    def show_welcome(self):
        ...
        UsbDriveIcon(center, size=96, bg=COLOR_BG).pack(pady=(0, 10))
    ...
    # ---------------- Step 3: Apps & Games ----------------
    def go_step_apps(self):
        self.clear()
        outer = tk.Frame(self.container, bg=COLOR_BG)
        outer.pack(fill="both", expand=True, padx=36, pady=28)

        StepDots(outer, 2).pack(anchor="w", pady=(0, 22))
        tk.Label(outer, text="Anything else? (optional)", font=FONT_H2, bg=COLOR_BG, fg=COLOR_TEXT)\
            .pack(anchor="w")
        tk.Label(outer,
                 text="CleanSlate can scan this computer for installed apps and games,\n"
                      "so you have a shopping list for after your fresh start.",
                 font=FONT_BODY, bg=COLOR_BG, fg=COLOR_MUTED, justify="left").pack(anchor="w", pady=(2, 16))

        self.apps_status_label = tk.Label(outer, text="", font=FONT_BODY_BOLD,
                                           bg=COLOR_BG, fg=COLOR_TEXT)
        self.apps_status_label.pack(anchor="w", pady=(0, 10))

        action_row = tk.Frame(outer, bg=COLOR_BG)
        action_row.pack(anchor="w", pady=(0, 12))
        self.scan_button = PillButton(action_row, "Scan This Computer", command=self.start_app_scan,
                                       width=220, height=48, bg=COLOR_BG)
        self.scan_button.pack(side="left")
        self.select_all_apps_button = PillButton(action_row, "Select All", kind="ghost",
                                                   command=self.select_all_apps, width=130, height=48, bg=COLOR_BG)
        self.clear_all_apps_button = PillButton(action_row, "Clear All", kind="ghost",
                                                  command=self.clear_all_apps, width=130, height=48, bg=COLOR_BG)

        self.apps_list_holder = tk.Frame(outer, bg=COLOR_BG)
        self.apps_list_holder.pack(fill="both", expand=True)

        self.apps_count_label = tk.Label(outer, text="", font=FONT_SMALL, bg=COLOR_BG, fg=COLOR_MUTED)
        self.apps_count_label.pack(anchor="w", pady=(6, 0))

        btn_row = tk.Frame(outer, bg=COLOR_BG)
        btn_row.pack(fill="x", pady=(16, 0))
        PillButton(btn_row, "Back", kind="ghost", command=self.go_step_files,
                   width=120, height=46, bg=COLOR_BG).pack(side="left")
        PillButton(btn_row, "Next", command=self.go_step_backup, width=140, height=46,
                   bg=COLOR_BG).pack(side="right")

        if self.available_apps:
            self.scan_button.set_text("Rescan")
            self.select_all_apps_button.pack(side="left", padx=(10, 0))
            self.clear_all_apps_button.pack(side="left", padx=(10, 0))
            self.apps_status_label.config(text=f"Found {len(self.available_apps)} app(s) & game(s).")
            self.render_apps_list()
        else:
            self.apps_status_label.config(text="Not scanned yet — totally optional.")

    def start_app_scan(self):
        self.scan_button.disable()
        self.apps_status_label.config(text="Scanning your computer for installed apps\u2026")
        self._apps_scan_queue = queue.Queue()
        threading.Thread(target=self._apps_scan_worker, daemon=True).start()
        self.after(100, self._poll_apps_scan)

    def _apps_scan_worker(self):
        try:
            apps = get_installed_apps()
            self._apps_scan_queue.put(("done", apps))
        except Exception as e:
            self._apps_scan_queue.put(("error", str(e)))

    def _poll_apps_scan(self):
        try:
            kind, payload = self._apps_scan_queue.get_nowait()
        except queue.Empty:
            self.after(100, self._poll_apps_scan)
            return
        ...
        if kind == "done":
            self.available_apps = payload
            self.scan_button.set_text("Rescan")
            if not self.available_apps:
                self.apps_status_label.config(text="No installed apps found on this computer.")
            else:
                self.apps_status_label.config(text=f"Found {len(self.available_apps)} app(s) & game(s).")
                self.select_all_apps_button.pack(side="left", padx=(10, 0))
                self.clear_all_apps_button.pack(side="left", padx=(10, 0))
            ...
            self.apps_status_label.config(text="Couldn't scan for apps on this computer.")

    def render_apps_list(self):
        for w in self.apps_list_holder.winfo_children():
            w.destroy()
        if not self.available_apps:
            return

        canvas = tk.Canvas(self.apps_list_holder, bg=COLOR_BG, highlightthickness=0, bd=0, height=260)
        canvas.pack(side="left", fill="both", expand=True)
        scroll_frame = tk.Frame(canvas, bg=COLOR_BG)
        scroll_frame.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))
        canvas.create_window((0, 0), window=scroll_frame, anchor="nw")

        apps_only = sorted((a for a in self.available_apps if a["category"] == "app"),
                            key=lambda a: a["name"].lower())
        games_only = sorted((a for a in self.available_apps if a["category"] == "game"),
                             key=lambda a: a["name"].lower())

        if apps_only:
            tk.Label(scroll_frame, text=f"Apps ({len(apps_only)})", font=FONT_BODY_BOLD,
                     bg=COLOR_BG, fg=COLOR_BLUE_HINT).pack(anchor="w", pady=(4, 6))
            for app in apps_only:
                self._make_app_row(scroll_frame, app)

        if games_only:
            tk.Label(scroll_frame, text=f"Games ({len(games_only)})", font=FONT_BODY_BOLD,
                     bg=COLOR_BG, fg=COLOR_BLUE_HINT).pack(anchor="w", pady=(14, 6))
            for game in games_only:
                self._make_app_row(scroll_frame, game)

        self.update_apps_count_label()

    def _make_app_row(self, parent, app):
        row = tk.Frame(parent, bg=COLOR_BG)
        row.pack(fill="x", pady=2)

        box = tk.Canvas(row, width=24, height=24, bg=COLOR_BG, highlightthickness=0, bd=0, cursor="hand2")
        pts = _round_rect_points(1, 1, 23, 23, 6)
        checked = app["name"] in self.selected_app_names
        rect = box.create_polygon(pts, smooth=True,
                                   fill=COLOR_PRIMARY if checked else COLOR_CARD,
                                   outline=COLOR_BLUE_HINT if checked else COLOR_CARD_BORDER, width=1.5)
        mark = box.create_text(12, 12, text="\u2713" if checked else "", fill="white", font=FONT_SMALL)

        def toggle(_e=None):
            if app["name"] in self.selected_app_names:
        ...
        label = tk.Label(row, text=app["name"], font=FONT_BODY, bg=COLOR_BG, fg=COLOR_TEXT,
                          anchor="w", cursor="hand2")
        label.pack(side="left")
        label.bind("<Button-1>", toggle)

        extra_bits = [b for b in (app.get("publisher"), app.get("version")) if b]
        if extra_bits:
            tk.Label(row, text="   " + "  \u2022  ".join(extra_bits), font=FONT_SMALL,
                     bg=COLOR_BG, fg=COLOR_MUTED, anchor="w").pack(side="left")

    def update_apps_count_label(self):
        n = len(self.selected_app_names)
        self.apps_count_label.config(
            text=f"{n} selected" if n else "None selected — that's fine, this step is optional.")

    def select_all_apps(self):
        self.selected_app_names = {a["name"] for a in self.available_apps}
        self.render_apps_list()
    ...
    def go_step_backup(self):
        ...
        tk.Label(outer, text=f"{file_count:,} files \u2022 {human_size(self.total_size)} "
                 ...
                 font=FONT_BODY, bg=COLOR_BG, fg=COLOR_MUTED).pack(anchor="w", pady=(2, 4))

        if self.selected_app_names:
            tk.Label(outer, text=f"+ a list of {len(self.selected_app_names)} app(s)/game(s) to reinstall",
                     font=FONT_SMALL, bg=COLOR_BG, fg=COLOR_MUTED).pack(anchor="w", pady=(0, 20))
        else:
            tk.Label(outer, text="", font=FONT_SMALL, bg=COLOR_BG).pack(anchor="w", pady=(0, 20))
        ...
        self.back_button_step3 = PillButton(btn_row, "Back", kind="ghost", command=self.go_step_apps,
    ...
    def _copy_worker(self):
        ...
        try:
            ...
            self._write_apps_list()

            self.progress_queue.put(("done", done_count, total_count))
        except Exception as e:
            self.progress_queue.put(("error", str(e)))

    def _write_apps_list(self):
        if not self.selected_app_names:
            return
        try:
            selected = [a for a in self.available_apps if a["name"] in self.selected_app_names]
            apps_only = sorted((a for a in selected if a["category"] == "app"), key=lambda a: a["name"].lower())
            games_only = sorted((a for a in selected if a["category"] == "game"), key=lambda a: a["name"].lower())

            lines = ["APPS & GAMES TO REINSTALL", "=" * 25,
                     f"Made for you by CleanSlate on {datetime.date.today().isoformat()}", ""]
            if apps_only:
                lines.append("Apps:")
                for a in apps_only:
                    bits = [a["name"]]
                    ...
                    if a.get("publisher"):
                        bits.append(f"- {a['publisher']}")
                    lines.append("  [ ] " + " ".join(bits))
                lines.append("")
            if games_only:
                lines.append("Games (Steam remembers these once you sign back in - just reinstall Steam):")
                for g in games_only:
                    lines.append(f"  [ ] {g['name']}")
                lines.append("")
            lines.append(f"-- CleanSlate v{APP_VERSION}")

            apps_list_path = os.path.join(self.backup_dest_folder, "Apps-To-Reinstall.txt")
            with open(apps_list_path, "w", encoding="utf-8") as f:
                f.write("\n".join(lines))
        except Exception:
            pass
    ...
    def go_step_checklist(self):
        ...
        StepDots(outer, 4).pack(anchor="w", pady=(0, 18))
