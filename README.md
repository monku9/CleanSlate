...
No accounts, no cloud, no confusing options — just a simple wizard
...
CleanSlate walks you through five simple steps:
...
| **3. Pick your apps** *(optional)* | Tap **Scan This Computer** and CleanSlate finds every installed app (and, on Windows, your Steam games) so you can check off which ones to remember. This doesn't copy the apps themselves — it saves a plain-text shopping list (`Apps-To-Reinstall.txt`) onto your drive so you know exactly what to reinstall afterward. |
| **4. Back it up** | A big, clear rounded progress bar shows exactly what's copying. Cancel anytime. Nothing is deleted from your device — this only *copies* files. |
| **5. Fresh Start** | On Windows, choose **Reinstall Windows** or **Switch to Linux**. Either way you get a tappable checklist for before, during, and after, plus a one-tap link to the right official resource. A copy of the matching checklist is also saved right onto your flash drive. |
...
## 📋 Why apps get *listed*, not copied

It might seem like CleanSlate should just copy your `Program Files` folder
over, the same way it copies your Desktop or Documents. It doesn't, on
purpose — most installed apps rely on registry entries, background
services, and drivers that a plain file copy can't bring back. Copying
the folder gives you a folder that doesn't run.

What actually works is: CleanSlate reads the same install list Windows'
own **Settings > Apps** page uses, lets you check off what you care about,
and saves it as a plain-text checklist on your drive. On Windows, it also
finds your installed Steam games — those don't even need listing to come
back, since Steam remembers your whole library the moment you sign back
in, but it's still handy to have the reminder.
